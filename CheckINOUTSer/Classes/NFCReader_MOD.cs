...
public class NFCReader
{
    ...
    //string readername = "ACS ACR122 0"; // NFC reader type
    private Card.SCARD_READERSTATE[] states;

    private bool statusSwitch = false;
    internal void establishContext() // khi truoc khi an nut botton
    {
    ...
    public string GetCardUID()
    {
        ...
        if (status != Card.SCARD_S_SUCCESS)
            ...
            cardUID = BitConverter.ToString(receivedUID.Take(4).ToArray()).Trim();
        return cardUID;
    }
    ...
    */
    ...
    public bool Connect()
    {
        if(string.IsNullOrEmpty(readername) || retCode == -2146435042)
            try
            {
    ...
    public void SelectDevice()
    {
        ...
        this.RdrState = new Card.SCARD_READERSTATE();
        readername = availableReaders[0].ToString();//selecting first device  string readername = "ACS ACR122 0";  
        this.RdrState.RdrName = readername;
    }

    public void CardStatusChanged()
    {
        if(hCard != 0 && readername != null)
        {
            retCode = Card.SCardGetStatusChange(hContext, 0, ref RdrState, 1);
